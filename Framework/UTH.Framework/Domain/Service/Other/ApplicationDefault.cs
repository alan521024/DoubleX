namespace UTH.Framework
{
        ...
        where TRep : IRepository<TEntity, TId>
    {
        ...
        public virtual List<TOutput> Query(QueryInput query)
        {
            ...
            var entitys = repository.Find(query.Size, where: predicate, sorting: query.Sorting);
            return EngineHelper.Map<List<TOutput>>(entitys);
        }
        ...
        {
            var entitys = repository.Find(top: top, where: predicate, sorting: Sorting);
            return EngineHelper.Map<List<TOutput>>(entitys);
        }
        ...
        {
            var result = await repository.FindAsync(top: top, where: predicate, sorting: Sorting);
