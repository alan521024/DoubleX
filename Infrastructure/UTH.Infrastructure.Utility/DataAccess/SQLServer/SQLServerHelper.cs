...
namespace UTH.Infrastructure.Utility
{
    ...
    public class SQLServerHelper
    {
        ...
        public static object ExecuteScalar(string connectionString, string commandText, CommandType commandType = CommandType.Text, params SqlParameter[] commandParameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand();
        ...
        public static TEntity ExecuteScalar<TEntity>(string connectionString, string commandText, CommandType commandType = CommandType.Text, params SqlParameter[] commandParameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand();
        ...
        // SqlDataReader返回一个连接，所以不能进行conn释放，在外界代码中使用完DataReader后，注意需要释放reader对象
        ...
        /// <param name="commandParameters">SQL参数对象</param>
        /// <returns></returns>
        public static SqlDataReader ExecuteReader(ref SqlConnection connection, string connectionString, string commandText, CommandType commandType = CommandType.Text, params SqlParameter[] commandParameters)
        {
        ...
        /// <param name="commandParameters">SQL参数对象</param>
        /// <returns></returns>
        public static IEnumerable<TEntity> ExecuteQuery<TEntity>(string connectionString, string commandText, CommandType commandType = CommandType.Text, Func<IDataReader, TEntity> func = null, params SqlParameter[] commandParameters)
        {
        ...
        public static bool ExecuteTransaction(string connectionString, List<string> commandTextes, List<SqlParameter[]> commandParameterses, CommandType commandType = CommandType.Text)
        {
            bool flag = false;
        ...
        /// <param name="commandType">默认执行类型</param>
        /// <returns>事务执行是否成功</returns>
        public static bool ExecuteTransaction(SqlConnection connection, List<string> commandTextes, List<SqlParameter[]> commandParameterses, CommandType commandType = CommandType.Text)
        {
            bool flag = false;
            if (commandTextes.Count == commandParameterses.Count)
            {
                ...
                {
                    commands.Add(new SqlCommand(x) { CommandType = commandType });
                });
        ...
        /// <param name="commandParameterses">sql语句列表对应的参数列表，参数列表必须与sql语句列表匹配</param>
        /// <returns></returns>
        public static bool ExecuteTransaction(string connectionString, List<SqlCommand> commands, List<SqlParameter[]> commandParameterses)
        {
            bool flag = false;
        ...
        /// <param name="commandParameterses">sql语句列表对应的参数列表，参数列表必须与sql语句列表匹配</param>
        /// <returns>事务执行是否成功</returns>
        public static bool ExecuteTransaction(SqlConnection connection, List<SqlCommand> commands, List<SqlParameter[]> commandParameterses)
        {
            bool flag = false;
            if (commands.Count == commandParameterses.Count)
            {
        ...
        private static void PrepareCommand(SqlConnection conn, SqlCommand cmd, string commandText, CommandType commandType, SqlTransaction trans, params SqlParameter[] cmdParms)
        {
